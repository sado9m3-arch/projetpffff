...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (req.method === "POST" && action === "complaints") {
      const { title, description, client_id }: CreateComplaintRequest = await req.json();
    ...
    if (req.method === "PUT" && action === "complaints") {
      const { id, status, fournisseur_id }: UpdateComplaintRequest = await req.json();
      ...
      if (fournisseur_id !== undefined) updateData.fournisseur_id = fournisseur_id;
    ...
    if (req.method === "DELETE") {
      ...
      const response = await fetch(`${supabaseUrl}/rest/v1/complaints?id=eq.${complaintId}`, {
        method: "DELETE",
        headers: {
          "Authorization": `Bearer ${supabaseServiceKey}`,
          "apikey": supabaseServiceKey,
      ...
      return new Response(
        JSON.stringify({ success: true, message: "Complaint deleted successfully" }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        }
      );
